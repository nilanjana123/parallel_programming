...
// printf("\n_______________________________________________________\n");
    ...
    for(auto s1:setP){
        int s2merger =0;
        set<set<int> >::iterator it = setP.begin();it++;
    // for(it ; it != setP.end();it++)
    for(auto s2:setP){
    // {   set<int> s2 = *it;
        set<int> intersect;
        ...
        // cout<<"empty";
        set<int> unioned;
        // set_union(s1.begin(), s1.end(), s2.begin(), s2.end(),std::inserter(unioned, unioned.begin()));
